...
void ir_dvice_init(void) {
  ...
  bl_gpio_enable_output(HXD039B2_POWER_CTRL_GPIO, true,
                        false); // 使能HXD039B2 控制IO
  ...
  hxd_power_off_timer = xTimerCreate("power off", pdMS_TO_TICKS(100), pdTRUE,
                                     (void *)0, hxd_power_timers_cb);
#endif
  /*初始化缓冲器*/
...
static void vTimerCallback(TimerHandle_t xTimer) {
  ...
  if (__hxd039b_busy) {
    // 学习成功，关闭HXD039B电源
    blog_info("Learn success,ac code 0x%02X 0x%02X", ac_codeGrud[0],
              ac_codeGrud[1]);
...
/**
 * @brief
 * 启动学习，运行此函数之后，对着红外接收头，按遥控器的开关键即可匹配红外码
 *
 */
void ir_codec_start_learn(void) {
  ...
  hxd_busy_timer = xTimerCreate("busy timer", pdMS_TO_TICKS(200), pdTRUE,
                                (void *)time_out_cont, vTimerCallback);
#endif
...
void ir_codec_set_power(int power_state) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        power_state ? IR_CODE_BYTE_AC_ON : IR_CODE_BYTE_AC_OFF};
...
void ir_codec_set_mode(int mode) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        IR_CODE_BYTE_AC_MODE_AUTO + mode};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
...
void ir_codec_set_temperature(unsigned char temperature) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        (temperature - 16) + IR_CODE_BYTE_AC_TEMPERATURE_16};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
...
void ir_codec_set_fan_mode(unsigned char fan_mode) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        fan_mode + IR_CODE_BYTE_AC_FAN_MODE_AUTO};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
...
void ir_codec_set_trend(unsigned char trend) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        trend + IR_CODE_BYTE_AC_TREND_UP};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
...
void ir_codec_set_trend_auto(unsigned char trend_auto) {
  ...
  uint8_t ir_code[5] = {IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE,
                        ac_codeGrud[0], ac_codeGrud[1],
                        trend_auto ? IR_CODE_BYTE_AC_TREND_AUTO_ON
                                   : IR_CODE_BYTE_AC_TREND_AUTO_OFF};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
...
void ir_codec_set_light_power(unsigned char light_power) {
  ...
  // 组织数据 例如打开空调： 0x30  0x06	0x03	0x03e	 81
  uint8_t ir_code[5] = {
      IR_CODE_BYTE_HANDLE, IR_CODE_BYTE_AC_TYPE, ac_codeGrud[0], ac_codeGrud[1],
      light_power ? IR_CODE_BYTE_AC_LIGHT_ON : IR_CODE_BYTE_AC_LIGHT_OFF};
  hxd_039b_send_data(ir_code, 5);
  // 延时等待发送完成后关闭HXD039B的电源
  //  hxd_039b2_delay_ms(HXD_039B2_START_TIME_MS*20);
#ifdef MCU_AI_WB2
 ...
 * @return false 不为空
 */
static bool isEmpty(CircularBuffer *cb) { return cb->count == 0; }
/**
 * @brief 判断缓冲器是否已满？
 *
 * @param cb 缓冲器枚举
 ...
 * @return false 没满
 */
static bool isFull(CircularBuffer *cb) { return cb->count == BUFFER_SIZE; }
/**
 * @brief 向缓冲区添加数据
 *
 * @param cb  缓冲器枚举
